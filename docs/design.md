...
| 企業ユーザー | 一般ユーザーの操作、自社情報・自社施工事例の管理 |

企業権限は `company_members` で判断する。ブラウザから送信された role や company ID を信用せず、server 側で所属と所有権を検査する。
...
| ユーザーダッシュボード | `/mypage` | 一般以上 | プロフィールと利用導線 |
| お気に入り一覧 | `/mypage/favorites` | 一般以上 | 一覧・解除 |
